...
void GSPlay::EnemySpawn(std::vector<std::shared_ptr<Enemy>>& enemies, std::vector<bool>& activeStatus) {
	...
	for (int i = 0; i < enemies.size(); i++) {
		if (!activeStatus[i]) {
			...
			enemies[i]->SetEnemyDirection(atan2(m_player->GetPlayerRandomPosCircle(50.f).y - m_enemy->GetEnemyPosition().y, m_player->GetPlayerRandomPosCircle(50.f).x - m_enemy->GetEnemyPosition().x));
			activeStatus[i] = true;
			break;
		}
	}
