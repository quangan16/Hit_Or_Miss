...
void GSPlay::EnemySpawn(std::vector<std::shared_ptr<Enemy>>& enemies, std::vector<bool>& activeStatus) {
	...
	if (isActiveAll) {
		...
		m_enemy->SetEnemyDirection((m_player->GetPlayerRandomPosCircle(300.f).y - m_enemy->GetEnemyPosition().y, m_player-> GetPlayerRandomPosCircle(300.f).x - m_enemy->GetEnemyPosition().x));
		m_enemy->SetSize(100, 100);
		enemies.push_back(m_enemy);
		activeStatus.push_back(true);
	}
